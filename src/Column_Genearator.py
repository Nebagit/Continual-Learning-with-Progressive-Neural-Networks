...
# My LSTM-based column generator for Progressive Neural Networks
class Column_generator_LSTM(ProgColumnGenerator):
    ...
    def create_column(self, device):
        ...
        if self.num_dens_Layer == 0:
        ...
        elif self.num_dens_Layer == 1:  # one extra dense layer
            ...
            columns.append(ProgDenseBlock(inSize=32, outSize=self.num_of_classes,
                                          activation=activation, numLaterals=0, drop_out=self.dropout))
        elif self.num_dens_Layer == 2:  # two extra dense layers
    ...
    def generateColumn(self, device, parent_cols):
        # Generates a column with lateral connections to parent columns
...
# My CNN-based column generator for Progressive Neural Networks
class Column_generator_CNN(ProgColumnGenerator):
    ...
    def create_column(self, device):
        # Create a new CNN column of blocks
        columns = []
        dropout = 0.2

        columns.append(ProgConv2DBNBlock(
            inSize=1, outSize=32, kernelSize=self.kernel_size,
            flatten=self.num_of_conv_layers == 1, numLaterals=0
        ))

        # Calculate dense layer input size based on conv layers
        dense_input_size = 32 * (config.pkt_size - (self.kernel_size - 1)) * (config.flow_size - 1 - (self.kernel_size - 1))
        if self.num_of_conv_layers == 2:
            columns.append(ProgConv2DBNBlock(
                inSize=1, outSize=16, kernelSize=self.kernel_size,
                flatten=True, numLaterals=0
            ))
            dense_input_size = 16 * (config.pkt_size - 4 * (self.kernel_size - 1)) * (config.flow_size - 1 - 2 * (self.kernel_size - 1))
        ...
        if self.num_dens_Layer == 0:
            ...
            columns.append(ProgDenseBlock(inSize=128, outSize=64,
                                          numLaterals=0, drop_out=dropout))
            columns.append(ProgDenseBlock(inSize=64, outSize=self.num_of_classes,
                                          activation=activation, numLaterals=0, drop_out=dropout))

        return ProgColumn(self.__genID(), columns, parentCols=[]).to(device)

    def generateColumn(self, device, parent_cols):
        # Generates a CNN column with lateral connections to parent columns
        new_column = self.create_column(device=device)
        for i in range(1, len(new_column.blocks)):
            for j in range(len(parent_cols)):
                new_column.blocks[i].laterals.append(nn.Linear(new_column.blocks[i].inSize, new_column.blocks[i].outSize))
